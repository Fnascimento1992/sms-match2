...
const processSmsMatching = async () => {
  try {
    const maxAttempts = 1;
    const disparoBatchSize = 50000; // Tamanho do lote de disparos
    const reciboBatchSize = 50000; // Tamanho do lote de recibos
    ...
    while (attempt < maxAttempts) {
      ...
      while (moreReceipts) {
        ...
        if (retornosTransformados.length === 0) {
          ...
          allRetornos = []; // Limpar variável para liberar memória
...
// Função auxiliar sleep para criar um timeout
